	...
	return candidates.filter((candidate) => {
		...
		return Boolean(
			hasCanonicalCandidateIdentity(candidate) &&
				...
				quote.priceImpactBps <= MAX_PRICE_IMPACT_BPS &&
				quoteAgeMs >= 0 &&
...
export function hasCanonicalExecutionAssetId(
	_chain: ExecutionRequest["chain"],
	assetId: string,
): boolean {
	return (
		Object.values(SOLANA_ASSET_REGISTRY).some(
			(asset) => asset.assetId === assetId,
		) ||
		(assetId.startsWith("sol:mainnet:") &&
			solanaAddressSchema.safeParse(assetId.slice("sol:mainnet:".length)).success)
	);
}
	...
	return {
		...
		warnings:
			missing.length === 0
				...
				: [
						...ranking.warnings,
						`${missing.length} supplied candidates were completed from Jupiter Solana discovery.`,
					],
	};
}
...
export function validateExecutionAssets(
	...
	const unavailableAssetIds = new Set(
		unavailableExecutionAssetIds(request, candidates),
	);
	const seen = new Set<string>();
	let total = 0n;
	...
	for (const selection of request.selections) {
		...
		if (amount < minTicket || amount % increment !== 0n) {
			throw new PolicyError(
				"INVALID_SLOT_SIZE",
				"Each allocation must be at least 0.10 USDC in 0.01 increments.",
			);
		}
		if (unavailableAssetIds.has(selection.assetId)) {
			throw new PolicyError(
				"ASSET_NOT_ELIGIBLE",
				`${selection.assetId} is not currently executable.`,
			);
...
export function unavailableExecutionAssetIds(
	request: ExecutionRequest,
	candidates: Candidate[],
): string[] {
