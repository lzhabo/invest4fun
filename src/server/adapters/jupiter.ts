...
export class JupiterProvider implements ExecutionProvider, CandidateProvider {
	...
	private readonly executableMints = new Set<string>();
	...
	async getRankingCandidates(
		...
		return assets.map(({ asset, token }, index) => ({
			...
			iconUrl: solanaAssetIconUrl(token.symbol, token.icon ?? undefined),
		}));
	}
	...
	async getCandidatesForDisplay(assetIds: string[]): Promise<Candidate[]> {
		...
		const missingMints = [
			...new Set(
				entries
					.map(({ mint }) => mint)
					.filter((mint) => !this.discoveredMetadata.has(mint)),
			),
		];
		if (missingMints.length) {
			try {
				await this.tokens(missingMints);
			} catch (error) {
				logProviderError("candidate_metadata", error);
			}
		}
		return entries.flatMap(({ assetId, mint }) => {
			const metadata = this.discoveredMetadata.get(mint);
			if (!metadata) return [];
			const candidate = this.candidateFromMetadata(assetId, metadata);
			return candidate
				? [
						this.executableMints.has(candidate.contract)
	...
	async prepareBasket(
		...
		for (const maxAccounts of accountProfiles) {
			...
			const unavailableAssetIds = preparedResults.flatMap((result, index) => {
				if (
					result.status === "rejected" &&
					result.reason instanceof ExecutionProviderError &&
					result.reason.code === "INVALID_TRANSACTION" &&
					result.reason.upstreamReason?.includes("No routes found")
				) {
					const assetId = request.selections[index]?.assetId;
	...
	private async resolveCandidates(
		...
		for (const assetId of assetIds) {
			try {
				const stub = await this.resolveCandidateMetadata(assetId);
				if (!stub) continue;
				...
				this.executableMints.add(stub.contract);
	...
	private async resolveCandidateMetadata(
		...
		const metadata =
			this.discoveredMetadata.get(mint) ?? (await this.token(mint));
		return this.candidateFromMetadata(assetId, metadata);
	}

	private candidateFromMetadata(
		assetId: string,
		metadata: JupiterToken,
	): Candidate | undefined {
		const asset =
			solanaAssetById(assetId) ??
			this.discoveredAssets.get(assetId) ??
			this.assetFromToken(metadata);
		if (
		...
		return candidateFromAsset(asset, metadata);
	}
	...
	private async build(
		...
		while (true) {
			...
			const retryRejectedBuild =
				...
				!responseReason?.includes("No routes found") &&
	...
	private async tokens(mints: string[]): Promise<JupiterToken[]> {
		let response: Response | undefined;
		...
		const tokens = ((await response.json()) as JupiterToken[]).filter((token) =>
			requested.has(token.id),
		);
		for (const token of tokens) this.discoveredMetadata.set(token.id, token);
		return tokens;
	}
...
function candidateFromAsset(
	...
	return {
		...
		iconUrl: solanaAssetIconUrl(asset.symbol, metadata.icon ?? undefined),
