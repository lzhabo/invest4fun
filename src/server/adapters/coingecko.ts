...
export class CoinGeckoIconProvider implements AssetIconProvider {
	...
	async enrichRankingCandidates(
		...
		const onchainCandidates = candidates.filter(
			(candidate) => candidate.chain === "SOLANA" && candidate.contract,
		);
		const onchainRows = await this.onchainMarketRows(
			onchainCandidates.flatMap((candidate) => candidate.contract ?? []),
		);
			...
			classificationCandidates
				...
				.map(async (candidate) => {
					...
					try {
						return [
							onchainKey(contract),
							await this.onchainTokenInfo(contract),
						] as const;
					} catch {
						return undefined;
					}
	...
	async details(asset: RegistryAsset): Promise<AssetDetails | undefined> {
		...
		const telegram = socialHandle(body?.links?.telegram_channel_identifier);
		...
		const community = [
			...
			...(body?.links?.chat_url ?? []).flatMap((url, index) => {
				...
				return safeUrl
					? [
							{
								label: index ? `Community ${index + 1}` : "Community",
								url: safeUrl,
							},
						]
					: [];
	...
	private async onchainMarketRows(addresses: string[]) {
		if (!addresses.length) return [] as OnchainTokenRow[];
		const unique = [...new Set(addresses)].slice(0, 50);
		return this.cachedResponse(
			`solana-market:${[...unique].sort().join(",")}`,
			MARKET_CACHE_TTL_MS,
			async () => {
				const response = await this.fetcher(
					`${COINGECKO_API_URL}/onchain/networks/solana/tokens/multi/${encodeURIComponent(unique.join(","))}`,
					{
						headers: this.headers(),
						signal: AbortSignal.timeout(10_000),
					},
	...
	private async onchainTokenInfo(
		...
		return this.cachedResponse(
			`${cacheNamespace}:solana-info:${address}`,
			cacheTtlMs,
			async () => {
				const response = await this.fetcher(
					`${COINGECKO_API_URL}/onchain/networks/solana/tokens/${encodeURIComponent(address)}/info`,
					{
						headers: this.headers(),
						signal: AbortSignal.timeout(10_000),
					},
	...
	private async yahooHistory(
		...
		return this.cachedResponse(
			...
			async () => {
				const response = await this.fetcher(
					`${YAHOO_CHART_URL}/${encodeURIComponent(symbol)}?range=${range}&interval=1d&events=history`,
					{
						headers: {
		...
		const fromDate =
			period === "ALL"
				...
				: new Date(Date.now() - lookbackDays * 86_400_000)
						...
						.slice(0, 10);
		return this.cachedResponse(
			`nasdaq:${symbol}:${assetClass}:${fromDate}`,
			HISTORY_CACHE_TTL_MS,
			async () => {
				const response = await this.fetcher(
					`${NASDAQ_API_URL}/${encodeURIComponent(symbol)}/historical?assetclass=${assetClass}&fromdate=${fromDate}&limit=5000`,
					{
						headers: {
							Accept: "application/json",
							"User-Agent": "Investmade/1.0 market-history",
...
function dedupeLinks<T extends { label: string; url?: string }>(
	items: T[],
): T[] {
	return [
		...new Map(
...
function onchainKey(address: string) {
	return address;
}
...
function yahooRange(period: HistoryPeriod) {
	if (period === "ALL") return "max";
	if (period === "1H" || period === "1D") return "5d";
	return period.toLowerCase();
}

function nasdaqAssetClass(symbol: string): "etf" | "stocks" {
	return new Set(["SPY", "GLD", "SMH", "XLE", "XOP", "EWU", "EWG"]).has(
		symbol.toUpperCase(),
	)
		? "etf"
		: "stocks";
...
function hasPriceMovement(points: PricePoint[]) {
	return (
		points.length >= 2 &&
		points.some((point) => point.price !== points[0]?.price)
	);
}
...
function historyDays(period: HistoryPeriod): "1" | "7" | "30" | "90" | "365" {
