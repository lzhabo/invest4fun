...
export function ReceiptScreen({
	...
	if (!record) {
		return (
			<main className="empty-page">
				<h1>No receipt yet</h1>
				<p>
					Complete an investment to see its verified settlement details here.
				</p>
				<button
					type="button"
	...
	const totalLegs = record.plan.quotes.length || selected.length;
	...
	const settledDescription = demoMode
		? `${totalInput} was split across ${totalLegs} demo assets. No transaction was broadcast.`
...
export function executionSummary({
	...
	const swaps = `${swapCount} ${swapCount === 1 ? "swap" : "swaps"}`;
	if (demoMode) return `${swaps} simulated locally`;
	if (submissionMode === "BATCH") {
		return `One atomic ${providerLabel} transaction · ${swaps}`;
	}
	if (transactionCount > 0) {
		const transactions =
			transactionCount === 1
				? `One independent ${providerLabel} transaction`
				: `${transactionCount} independent ${providerLabel} transactions`;
		return `${transactions} · ${swaps}`;
	}
	return `Independent ${providerLabel} execution · ${swaps}`;
}
