...
function providerFor(
	...
	const fetcher = vi.fn(async (input: string | URL | Request) => {
		...
		if (url.includes("/tokens/v2/search")) {
			const mints = (new URL(url).searchParams.get("query") ?? "").split(",");
...
describe("Jupiter atomic Solana execution", () => {
	it("loads card metadata without building swap routes", async () => {
		...
		const candidates = await provider.getCandidatesForDisplay(assetIds);

		expect(candidates.map((candidate) => candidate.assetId)).toEqual(assetIds);
		expect(
			fetcher.mock.calls.filter(([input]) =>
				String(input).includes("/tokens/v2/search"),
			),
		).toHaveLength(1);
		expect(
	...
	it("keeps an exact-size routed candidate executable for review", async () => {
		...
		const routed = await provider.getCandidatesForExecution(
			wallet,
			[assetId],
			"25000000",
		);
		const displayed = await provider.getCandidatesForDisplay([assetId]);

		expect(routed[0]?.marketHealthy).toBe(true);
		expect(displayed[0]?.marketHealthy).toBe(true);
	});
	...
	it("keeps Solana discovery available when an optional Jupiter source fails", async () => {
		...
		expect(
			candidates.find((candidate) => candidate.symbol === "AAPLx")?.iconUrl,
		).toBe("https://xstocks-metadata.backed.fi/logos/tokens/AAPLx.png");
		expect(
			candidates.find((candidate) => candidate.symbol === "NVDAx")?.iconUrl,
		).toBe("https://xstocks-metadata.backed.fi/logos/tokens/NVDAx.png");
		expect(
			candidates.find((candidate) => candidate.symbol === "TSLAx")?.iconUrl,
		).toBe("https://xstocks-metadata.backed.fi/logos/tokens/TSLAx.png");
	});
